...
{#if apiKeys}
	{#each Object.keys(apiKeys) as key}
		<div>
			<img on:click={() => (showKeys = !showKeys)} class="dark:invert cursor-pointer h-6 w-6 inline align-baseline mr-3" src={eye} />
			{#if showKeys}
				<span>{key}</span>
			{:else}
				<span>{"~ ".repeat(key.length / 2)}</span>
			{/if}
			<img on:click={() => editKey(key)} class="dark:invert cursor-pointer h-6 w-6 inline align-baseline ml-3" src={pen} />
		</div>
	{/each}
{/if}
...
{#if scopeEditorOpen}
	<form on:submit|preventDefault={handleSubmit} action={"?/savekey"} class="flex modal items-center justify-center">
		<div class="bg-white relative p-5 flex flex-col gap-5 items-start">
			...
			<div class="inline-grid grid-cols-[auto_auto_auto] gap-3 gap-x-5 justify-items-center items-center">
				...
				{#each Object.entries(SCOPES.fields) as [field, attrs]}
					...
					{:else}&nbsp;{/if}
					{#if attrs.write}
						<input type="checkbox" checked={scopeFlagMap[field + ":write"]} name={field + ":write"} class="bg-neutral-300 rounded-full w-8 h-8" />
					{:else}&nbsp;{/if}
			...
			<div class="grid grid-cols-[auto_auto] gap-3 gap-x-5 justify-items-center items-center">
				{#each Object.entries(SCOPES.actions) as [action, description]}
					<input type="checkbox" checked={scopeFlagMap[action]} name={action} class="bg-neutral-300 rounded-full w-8 h-8" />
